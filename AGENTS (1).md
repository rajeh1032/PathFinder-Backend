...
The repository is currently a Node.js/Express CommonJS backend with a modular folder structure under `src/modules`. Many feature modules are scaffolded as empty files. The only runtime route currently mounted in `src/server.js` is `/test`.
...
Do not blindly follow older handoff recommendations when they conflict with current code or docs. The handoff mentions TypeScript, Zod, Prisma, and Supabase Auth, but the current repository is CommonJS JavaScript, Joi, Supabase JS client, and Node-owned auth.
...
- Current implemented base route: `/test`.
- Handoff/planned API prefix: `/api/v1`, but this is not yet implemented in `src/server.js`.
...
- Current config uses OpenAI via `src/config/openai.js`.
- Prompt files exist under `src/modules/ai/prompts/` but are currently empty.
- Feature services should call AI through shared AI/OpenAI service wrappers, not directly from controllers.
- Use deterministic code for scores when the feature design calls for deterministic matching; use OpenAI for explanations/generation.
